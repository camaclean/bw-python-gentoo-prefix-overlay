...
src_configure() {
	...
	if use cray; then
		...
		scalapack_inc="$(pkg-config --cflags-only-I lapack | perl -ne '/-I([\S]+)/ && print $1')"
		scalapack_lib="$(pkg-config --libs lapack | tr ' ' ',')"

		tpsl_inc="$CRAY_TPSL_PREFIX_DIR/lib"
		tpsl_lib="[$(pkg-config --libs cray-tpsl | tr ' ' ',')]"
		parmetis_inc="${tpsl_inc}"
		parmetis_lib="${tpsl_lib}"
		metis_inc="${tpsl_inc}"
		metis_lib="${tpsl_lib}"
		superlu_inc="${tpsl_inc}"
		superlu_lib="-lsuperlu"
		mumps_inc="${tpsl_inc}"
		mumps_lib="[-lcmumps,-ldmumps,-lsmumps,-lzmumps,-lmumps_common,-lpord]"
		scotch_inc="${tpsl_inc}"
		scotch_lib="[-lptesmumps,-lptscotch,-lptscotcherr,-lscotch,-lscotcherr]"
		hypre_inc="${tpsl_inc}"
		hypre_lib="-lHYPRE"
		#suitesparse_inc="[${CRAY_TPSL_PREFIX_DIR}/include,$(pkg-config --cflags-only-I cholmod | perl -ne '/-I([\S]+)/ && print $1')]"
		#suitesparse_lib="[-L${tpsl_inc},$(pkg-config --libs cholmod | tr ' ' ',')]"
	else
		MPICC=mpicc
		MPICXX=mpicxx
		MPIF77=mpif77
		...
		#suitesparse_lib="[$(pkg-config --libs cholmod | tr ' ' ',')]"
	fi
	fftw_inc="$(pkg-config --cflags-only-I fftw3 | perl -ne '/-I([\S]+)/ && print $1')"
	fftw_lib="[$(pkg-config --libs fftw3 | tr ' ' ',')]"
	...
	econf \
		...
		$(petsc_with superlu superlu \
			${superlu_inc} ${superlu_lib}) \
		$(petsc_with parmetis parmetis \
			${parmetis_inc} $parmetis_lib) \
		$(petsc_with X x) \
		$(petsc_with X x11) \
		$(petsc_with scotch ptscotch ${scotch_inc} ${scotch_lib}) \
		$(petsc_with_sys mumps scalapack \
			$scalapack_inc $scalapack_lib) \
		$(petsc_with mumps mumps ${mumps_inc} ${mumps_lib}) \
		--with-imagemagick=0 \
#		--with-mpirun=$mpirun \
#		--with-mpiexec=$mpiexec \
		$(petsc_with python) \
		$(petsc_with boost) \
		$(petsc_with fftw fftw ${fftw_inc} ${fftw_lib})
