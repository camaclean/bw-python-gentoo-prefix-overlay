...
IUSE="context cray debug doc icu +nls mpi prefix-chaining python static-libs +threads tools"
...
create_user-config.jam() {
	...
	if use mpi; then
		...
		if use cray ; then
			mpi_configuration="using mpi : : <find-shared-library>mpich ;"
			compiler="gcc"
			compiler_version=""
			compiler_executable="CC"
			CFLAGS="${CFLAGS} -fPIC -I\$MPICH_DIR/include"
			CXXFLAGS="${CXXFLAGS} -fPIC -I\$MPICH_DIR/include"
			LDFLAGS="${LDFLAGS} -L\$MPICH_DIR/lib"
		fi
	fi
...
src_configure() {
	...
	local icu_eprefix="${EPREFIX}" IFS_bak
	if use prefix-chaining && use icu; then
		IFS_bak=$IFS
		IFS=":"
		local p
		for p in ${EPREFIX} ${PORTAGE_READONLY_EPREFIXES}; do
			if portageq has_version "${p}" ">=dev-libs/icu-3.6:=[${MULTILIB_USEDEP}]"; then
				icu_eprefix="${p}"
				break;
			fi
		done
	...
	local boost_build_eprefix="${EPREFIX}"
	echo ${PORTAGE_READONLY_EPREFIXES}
	if use prefix-chaining; then
		IFS_bak=$IFS
		IFS=":"
		local p
		for p in ${EPREFIX} ${PORTAGE_READONLY_EPREFIXES}; do
			if portageq has_version "${p}" "=dev-util/boost-build-${MAJOR_V}*"; then
				boost_build_eprefix="${p}"
				break;
			fi
		done
		IFS=${IFS_bak}
	...
	OPTIONS+=(
		pch=off
		--boost-build="${boost_build_eprefix}"/usr/share/boost-build
		--prefix="${ED}usr"
		--layout=system
