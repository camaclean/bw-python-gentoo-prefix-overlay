...
IUSE="cray debug default-compiler-rt default-libcxx doc +static-analyzer
...
PATCHES=(
	...
	#"${FILESDIR}"/6.0.1/gentoo-prefix-cray.patch
)
...
src_prepare() {
	...
	if use cray; then
		sed -e "/@GENTOO_PORTAGE_EPREFIX@/d" -i lib/Frontend/InitHeaderSearch.cpp
		eapply "${FILESDIR}"/6.0.1/gentoo-prefix-cray.patch
		eprefixify lib/Driver/ToolChains/Gnu.cpp
	else
...
multilib_src_configure() {
	...
	if [[ -n ${EPREFIX} ]]; then
		if use cray; then
			mycmakeargs+=(
				-DGCC_INSTALL_PREFIX="${GCC_PATH}/snos"
			)
		else
			mycmakeargs+=(
