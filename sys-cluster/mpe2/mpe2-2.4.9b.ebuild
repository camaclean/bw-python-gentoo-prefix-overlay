...
SRC_URI="ftp://ftp.mcs.anl.gov/pub/mpi/${PN%2}/${MY_P}.tgz"
...
pkg_setup() {
	...
	if has_version sys-cluster/openmpi; then
		...
		export MPI_CFLAGS="${CFLAGS} -fPIC -isystem ${EPREFIX}/usr/include"
		export MPI_CXXFLAGS="${CXXFLAGS} -fPIC -isystem ${EPREFIX}/usr/include"
		export MPI_FFLAGS="${FFLAGS} -fPIC -isystem ${EPREFIX}/usr/include"
...
src_configure() {
	...
	if use cray; then
		...
		c="--with-mpicc=${EPREFIX}/usr/bin/mpicc"
	fi
	...
	if use fortran; then
		if use cray; then
			...
			c="${c} --with-mpif77=${EPREFIX}/usr/bin/mpif77"
		fi
	else
		c="${c} --disable-f77"
	fi
	...
	if use minimal; then
		...
		if use cray; then
			c="${c} --with-java2=${JAVA_HOME} --enable-slog2=build"
		else
	...
	econf ${c} \
		...
		--with-htmldir=${EPREFIX}/usr/share/${PN} \
		--with-docdir=${EPREFIX}/usr/share/${PN} \
		--enable-collchk \
		--enable-wrappers \
