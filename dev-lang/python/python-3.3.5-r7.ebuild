...
SRC_URI="http://dev.gentoo.org/~grobian/distfiles/python-prefix-3.3.3-gentoo-patches${PREFIX_PATCHREV}.tar.bz2
	http://www.python.org/ftp/python/${PV}/${MY_P}.tar.xz
...
IUSE="aqua build +crypt doc elibc_uclibc examples gdbm hardened ipv6 libressl +ncurses nis +readline sqlite +ssl +threads tk wininst +xml"
...
src_prepare() {
	...
	EPATCH_EXCLUDE="${excluded_patches}" EPATCH_SUFFIX="patch" \
		epatch "${WORKDIR}"/python-prefix-3.3.3-gentoo-patches${PREFIX_PATCHREV}
	...
	#epatch "${FILESDIR}"/${PN}-3.3-bwpy-platform.patch
...
src_install() {
	...
	if [[ ${CHOST} == *-aix* ]]; then
		...
		sed -e "/Libs:/s|\$| -Wl,-bE:${EPREFIX}/${libdir#$ED}/config-${PYVER}/python.exp -lld|" \
			-i "${ED}"usr/$(get_libdir)/pkgconfig/python-${PYVER}.pc || die "sed failed"
		sed -e "s|:Modules/python.exp|:${EPREFIX}/${libdir#$ED}/config-${PYVER}/python.exp|" \
			-e 's| -bI:| -Wl,-bI:|g' \
			-e '/LINKFORSHARED/s|-bE:|-bI:|g' \
			-e '/LINKFORSHARED/s| -lld||g' \
			-i "${libdir}"/{_sysconfigdata.py,config-${PYVER}/Makefile} || die "sed failed"
	fi
