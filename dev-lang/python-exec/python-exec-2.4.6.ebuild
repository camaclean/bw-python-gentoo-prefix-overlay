...
pkg_preinst() {
	local py p pyimpls=() i EPYTHON ifs_bak
	if use prefix-chain; then
		...
		for py in ${pyimpls[@]}; do
			if $(get_eprefix dev-lang/python:${py#python} RDEPEND); then
				if [[ $(get_eprefix dev-lang/python:${py#python} RDEPEND) != "${EPREFIX}" ]]; then
				echo "setting up symlinks for ${py}"
					for f in python{,2,3,2.7,3.3,3.4,3.5,3.6,3.7} python{,2,3,2.7,3.3,3.4,3.5,3.6,3.7}-config 2to3 idle pydoc pyvenv; do
						for p in ${READONLY_R_EPREFIXES/:/ /}; do
							if [ -e "${p}"/usr/lib/python-exec/${py}/${f} ]; then
								mkdir -p "${ED%/}"/usr/lib/python-exec/${py}
								ln -snf "${p%/}"/usr/lib/python-exec/${py}/${f} "${ED%/}"/usr/lib/python-exec/${py}/${f} || die
								break
							fi
						done
					done
		...
		PYTHON_EXEC_CONFIG_EPREFIX=
		for p in ${READONLY_R_EPREFIXES/:/ /}; do
			if [[ -e "${p}"/etc/python-exec/python-exec.conf ]]; then
				PYTHON_EXEC_CONFIG_EPREFIX="${p}"
				break
			fi
		done
	fi
	...
	elif [[ -n "${PYTHON_EXEC_CONFIG_EPREFIX}" ]]; then
		cp "${PYTHON_EXEC_CONFIG_EPREFIX}"/etc/python-exec/python-exec.conf \
			"${ED%/}"/etc/python-exec/python-exec.conf || die
	else
