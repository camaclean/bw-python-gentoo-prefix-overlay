...
pkg_preinst() {	
	...
	for py in ${pyimpls[@]}; do
		for f in python{,2,3,2.7,3.3,3.4,3.5} python{,2,3,2.7,3.3,3.4,3.5}-config 2to3 idle pydoc pyvenv; do
			if [ ! -e "${EPREFIX}"/usr/lib/python-exec/${py}/${f} ]; then
				for p in ${PORTAGE_READONLY_EPREFIXES/:/ /}; do
					if [ -e "${p}"/usr/lib/python-exec/${py}/${f} ]; then
						mkdir -p "${EPREFIX}"/usr/lib/python-exec/${py}
						ln -snf "${p}"/usr/lib/python-exec/${py}/${f} "${EPREFIX}"/usr/lib/python-exec/${py}/${f}
						break
					fi
				done
	...
	elif [[ ! -z "${PYTHON_EXEC_CONFIG_EPREFIX}" ]]; then
