...
inherit eutils multilib libtool prefix
...
IUSE="prefix-chaining cray"

RDEPEND="sys-apps/gentoo-functions"
DEPEND="${RDEPEND}"

S=${WORKDIR}/baselayout-${PV}
...
src_prepare() {
	echo "${S}"
	cd "${S}"
	epatch "${FILESDIR}"/baselayout-prefix-${PV}-1-profile-eprefixify.patch
	if use prefix-chaining
	then
		epatch "${FILESDIR}"/baselayout-prefix-${PV}-2-profile-prefix-chaining.patch
		...
		if use cray
		then
			epatch "${FILESDIR}"/baselayout-prefix-${PV}-3-profile-prefix-chaining-cray.patch
		fi
	else
		if use cray
		then
			epatch "${FILESDIR}"/baselayout-prefix-${PV}-2-nochain-cray.patch
		fi
	fi
	eprefixify etc/profile
}
