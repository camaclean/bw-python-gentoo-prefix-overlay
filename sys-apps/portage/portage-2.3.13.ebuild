...
SLOT="0"
IUSE="build doc environment-modules epydoc +ipc linguas_ru +native-extensions selinux xattr prefix-chaining"
...
environment-modules? ( prefix-chaining )"
...
python_prepare_all() {
	...
	if use prefix-chaining; then
		...
		epatch "${FILESDIR}"/${PN}-2.3.5-prefix-chaining-additions.patch
	fi
	if use environment-modules; then
		epatch "${FILESDIR}"/${PN}-2.3.5-environment-modules.patch
	fi
