...
SLOT="0/${PV}"
IUSE="X blas boost cray cuda +doc -double-precision +fftw lapack mkl mpi +offensive openmp +single-precision test +threads +tng ${ACCE_IUSE}"
...
src_configure() {
	...
	for x in ${GMX_DIRS}; do
		...
		[[ ${x} = "float" ]] && use cuda && \
			cuda=( -DGMX_GPU=ON )
		if use cray && use cuda; then
			cuda+=( -DCUDA_HOST_COMPILER=$(which CC) )
		fi
		mycmakeargs=(
			${mycmakeargs_pre[@]} ${p}
			-DGMX_MPI=OFF
		...
		if use cray; then
			BUILD_DIR="${WORKDIR}/${P}_${x}_mpi" CC="cc" cmake-utils_src_configure
		else
