...
PATCHES=(
	...
	"${FILESDIR}/${PN}-2.2.3-jasper.patch"
	"${FILESDIR}/${PN}-2.3.0-jasper2.patch" #bug 599626
)
...
src_configure() {
	...
	local myconf=(
		...
		--with-libjson-c="$(get_eprefix dev-libs/json-c RDEPEND)/usr/"
		--with-libtiff
		--with-libtool
		--with-libz="$(get_eprefix sys-libs/zlib RDEPEND)/usr/"
		--with-gnm
		--without-bsb
		--without-dods-root
		--without-ecw
		...
		$(use_with mysql mysql "$(get_eprefix dev-db/mysql-connector-c RDEPEND)"/usr/bin/mysql_config)
		$(use_with netcdf)
		$(use_with oracle oci)
		$(use_with odbc)
		$(use_with ogdi ogdi "$(get_eprefix sci-libs/ogdi RDEPEND)"/usr)
		$(use_with opencl)
		...
		$(use_with python)
		$(use_with spatialite)
		$(use_with sqlite sqlite3 "$(get_eprefix dev-db/sqlite RDEPEND)"/usr)
		$(use_with threads)
		$(use_with webp)
		$(use_with xls freexl) )
...
src_install() {
	...
	if use python; then
		...
		rm -rf "${D}"/lib # wtf is python3.5 doing?
	fi
