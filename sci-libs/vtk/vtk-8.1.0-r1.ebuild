...
inherit distutils-r1 flag-o-matic java-pkg-opt-2 qmake-utils versionator toolchain-funcs cmake-utils virtualx webapp
...
SLOT="0"
IUSE="
	all-modules aqua boost cray doc examples imaging ffmpeg gdal java json kaapi mpi
...
src_prepare() {
	epatch "${FILESDIR}/vtk-8.1.0-gentoo.patch"
	use cray && epatch "${FILESDIR}/vtk-7.1.0-cray-netcdf.patch"
...
python_configure() {
	...
	local mycmakeargs=(
		...
		-DVTK_USE_SYSTEM_WSLINK=ON
		#-DVTK_USE_SYSTEM_GLEW=ON
		-DVTK_USE_SYSTEM_LIBPROJ4=ON
		-DVTK_USE_SYSTEM_MPI4PY=ON
		#-DZLIB_ROOT="$(get_eprefix sys-libs/zlib RDEPEND)"/usr
	...
	# IO
		...
		-DGDAL_INCLUDE_DIR="$(get_eprefix sci-libs/gdal DEPEND)/usr/include/gdal"
		-DModule_vtkIOGeoJSON=$(usex json)
		-DModule_vtkIOXdmf2=$(usex xdmf2)
		-DBUILD_TESTING=$(usex examples)
		-DPNG_LIBRARIES="$(get_eprefix media-libs/libpng:0 RDEPEND)/usr/lib/libpng16.so;z"
		-DPNG_INCLUDE_DIRS="$(get_eprefix media-libs/libpng:0 RDEPEND)"/usr/include
		-DVTK_VPIC_USE_MPI=$(usex mpi)
	...
	if use python; then
		mycmakeargs+=(
			-DCMAKE_INSTALL_PREFIX="/_${EPYTHON}/${EPREFIX}/usr"
			-DVTK_INSTALL_PYTHON_MODULE_DIR="$(python_get_sitedir)"
			-DPYTHON_INCLUDE_DIR="$(python_get_includedir)"
			-DPYTHON_LIBRARY="$(python_get_library_path)"
			-DSIP_PYQT_DIR="$(get_eprefix dev-python/PyQt5)/usr/share/sip"
	...
	if use cray; then
		...
		mycmakeargs+=( -DNETCDF_CXX_EPREFIX="$(get_eprefix sci-libs/netcdf-cxx)" )
	fi
	...
	sed -e 's/VTKFILTERSPYTHONPY[23]/VTKFILTERSPYTHON/g' "${BUILD_DIR}"/Filters/Python/vtkFiltersPythonPy?Module.h > "${BUILD_DIR}"/Filters/Python/vtkFiltersPythonModule.h || die
}
...
src_configure() {
	if use python; then
		...
		python_foreach_impl do_python_configure
	else
		python_configure
	fi
}
...
src_compile() {
	if use python; then
		...
		python_foreach_impl do_python_compile;
	else
		cmake-utils_src_compile;
	fi
}
...
src_install() {
	...
	if use python; then
		do_python_install()
		{
			...
			mkdir -p ${D%/}/_${EPYTHON}/${EPREFIX}/usr/lib/python-exec/${EPYTHON} || die
			pushd ${D%/}/_${EPYTHON}/${EPREFIX}/usr/bin || die
			local i root=${D%/}/_${EPYTHON}
			for i in *; do
				mv ${i} ${D%/}/_${EPYTHON}/${EPREFIX}/usr/lib/python-exec/${EPYTHON}/${i} || die
				dodir /usr/bin
		...
		python_foreach_impl do_python_install
	else
		cmake-utils_src_install;
	fi
	cd "${S}" || die
