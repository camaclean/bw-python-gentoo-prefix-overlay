...
SLOT="0"
IUSE="
	all-modules aqua boost cg cray doc examples imaging ffmpeg gdal java json kaapi mpi
...
src_prepare() {
	use cray && epatch "${FILESDIR}/${P}-cray-netcdf.patch"
	sed \
...
src_configure() {
	...
	#export CFLAGS="${CFLAGS} -I$(get_eprefix media-video/nvidia-settings)/usr/include/NVCtrl"
	#export CXXFLAGS="${CXXFLAGS} -I$(get_eprefix media-video/nvidia-settings)/usr/include/NVCtrl"
...
#
	...
	if use python; then
		mycmakeargs+=(
			...
			-DSIP_PYQT_DIR="$(get_eprefix dev-python/PyQt4)/usr/share/sip"
	...
	if use qt4; then
		export LDFLAGS="$LDFLAGS -Wl,--rpath=$(get_eprefix dev-qt/qtcore:4)/usr/lib/qt4/"
		mycmakeargs+=(
			-DVTK_USE_QVTK=ON
			...
			-DQT_INCLUDE_DIR="$(get_eprefix dev-qt/qtcore:4)/usr/include/qt4"
	...
	if use qt5; then
		mycmakeargs+=(
			...
			-DQT_INCLUDE_DIR="$(get_eprefix dev-qt/qtcore:5)/usr/include/qt5"
