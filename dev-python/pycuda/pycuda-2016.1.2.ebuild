...
python_prepare_all() {
	...
	sed \
		...
		-e "s:\"--cubin\":\'--cubin\', \'--compiler-bindir=$(cuda_gccdir)\', \'--linker-options=\"${rawldflags// /,},-L/opt/cray/nvidia/default/lib64,--rpath=/opt/cray/nvidia/default/lib64\"\':g" \
		-e "s:/usr/include/pycuda:${S}/src/cuda:g" \
		-i pycuda/compiler.py || die
	
	use cray && sed -e "s:\"nvcc\":\"$(which nvcc)\":g" -i pycuda/compiler.py || die
